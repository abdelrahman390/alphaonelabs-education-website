...
from web.models import (
    ...
    StudyGroup,
    Subject,
    Challenge,  # To add Challenge test data
    ChallengeSubmission,  # To add Challenge test data submission
)
...
################## new changes Start ##################
        # Create challenges
        now = timezone.now().date()  # Use date() for DateField compatibility
        challenges = []
        for i in range(5):  # Create 5 weekly challenges
            challenge = Challenge.objects.create(
                title=f"Weekly Challenge {i + 1}",
                description=f"Complete task {i + 1} for this week's challenge.",
                challenge_type="weekly",
                week_number=i + 1,  # Unique week numbers
                start_date=now - timedelta(days=(4 - i) * 7),  # Staggered past and future dates
                end_date=now - timedelta(days=(4 - i) * 7) + timedelta(days=6),
            )
            challenges.append(challenge)
            self.stdout.write(f"Created challenge: {challenge}")

        # Add a one-time challenge
        one_time_challenge = Challenge.objects.create(
            title="Special One-Time Challenge",
            description="A unique one-time challenge for all students.",
            challenge_type="one_time",
            week_number=None,  # No week number for one-time
            start_date=now - timedelta(days=2),
            end_date=now + timedelta(days=2),
        )
        challenges.append(one_time_challenge)
        self.stdout.write(f"Created one time challenge: {one_time_challenge}")

        # Create challenge submissions
        for student in students:
            # Randomly select 1-3 challenges for each student to submit to
            selected_challenges = random.sample(challenges, random.randint(1, 3))
            for challenge in selected_challenges:
                submission = ChallengeSubmission.objects.create(
                    user=student,
                    challenge=challenge,
                    submission_text=f"Submission for {challenge.title} by {student.username}",
                )
