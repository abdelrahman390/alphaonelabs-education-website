...
class Cart(models.Model):
    ...
    def total(self):
        for item in self.items.all():
            ...
            print(f"ID: {item.id}, Price: {item.final_price}")
...
class CartItem(models.Model):
    ...
    def final_price(self):
        ...
        return max(self.price - self.discount, 0)  # Fallback to original price

    def __str__(self):
        if self.course:
            print(f"{self.course.title} in cart for {self.cart}")
