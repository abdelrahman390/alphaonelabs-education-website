...
def share_discount(request):
    if request.method == "POST":
        try:
            ...
            if course_id:
                # ShareTracking.objects.create(platform=platform, course_url=course_url)
                print(course_id)
...
def index(request):
    ...
    # Get active one-time challenges
...
def create_course(request):
    if request.method == "POST":
        ...
        form = CourseForm()
    # field_list = ["description", "learning_objectives", "prerequisites"]
    # return render(request, "courses/create.html", {"form": form, "field_list": field_list})
...
def challenges_page(request):
    ...
    # Get current challenge
    current_challenge = Challenge.objects.filter(challenge_type='weekly',start_date__lte=timezone.now(), end_date__gte=timezone.now()).first()

    # Get active one-time challenges
    current_one_time_challenge = Challenge.objects.filter(
        challenge_type='one_time',
        start_date__lte=timezone.now(),
        end_date__gte=timezone.now()
    ).first()
    ...
    context = {
        "profile": profile,
        "current_challenge": current_challenge,
        "current_one_time_challenge": current_one_time_challenge,
        "form": form,
    }
    print("%%%%%%%%%%%", context)
    return render(request, "web/challenges_page.html", context)
...
@login_required
def challenge_submit(request, challenge_type, week_number=None):
    challenge = get_object_or_404(Challenge, week_number=week_number)
    # Check if the user has already submitted this challenge

    if week_number is not None:
        challenge = get_object_or_404(Challenge, challenge_type=challenge_type, week_number=week_number)
    else:
        challenge = get_object_or_404(Challenge, challenge_type=challenge_type)

    existing_submission = ChallengeSubmission.objects.filter(user=request.user, challenge=challenge).first()

    # todo remove this comment
    # if existing_submission:
    #     return redirect("challenge_detail", challenge_type)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # submission.challenge_type = challenge.challenge_type
            print("&&& challenge &&&&&", challenge)
            submission.save()
            messages.success(request, "Your submission has been recorded!")
            # if week_number:
            #     return redirect("challenge_detail",challenge_type=challenge_type, week_number=week_number)
            # else:
            #     return redirect("challenges_page",challenge_type=challenge_type)
    else:
